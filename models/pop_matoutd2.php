...
<div id="listing"></div>
	...
	<?php  
		$q = "SELECT * FROM matoutd mod 
			  INNER JOIN material m ON m.mat_id=mod.mat_id 
			  WHERE matout_id=".$_GET['matout_id'];
		//$q = "SELECT * FROM matoutd mod";
		//echo $q;
		$rec = $pdo1->query($q);
	...
	?>
		<tr>	
			...
			<td style="text-align:center;"><input border='0' type='image' src="img/expander.gif" onClick="insert_matoutd('<?php echo $uraian ?>','INDONESIA','','<?php echo $qty." ".$unit ?>','<?php //echo $price ?>')" /></td>
		</tr>
	<?php } ?>
