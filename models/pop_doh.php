...
$('#dtsampai').datepicker({ dateFormat: 'dd/mm/yy' });
	...
	$("#fcari").submit(function(){
		...
		try{
			...
			$("#listing").load('pop_doh_load.php',obj,function (){
...
<div id="listing"></div>
	...
	<?php 
		...
		$q = "SELECT *,doh.Number AS dohno,c.Number AS cno FROM tbdelorder_hd doh LEFT JOIN tbcustomer c ON c.Customer_Code=doh.Buyer ORDER BY Tanggal LIMIT 20";
		$rec = $pdovb->query($q);
		$rs = $rec->fetchAll(PDO::FETCH_ASSOC);		
		foreach ($rs as $r){
	?>
		<tr>	
			<td><?php echo $r['ID']//." ".$rsSup[0]['sup_name'] ?></td>
            <td><?php echo $r['Tanggal'] ?></td>
            <td><?php echo $r['Customer_Name'] ?></td>
            <td><?php echo number_format($r['Total_qty']) ?></td>																			
