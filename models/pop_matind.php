...
$('#dtsampai').datepicker({ dateFormat: 'dd/mm/yy' });
	...
	$("#fcari").submit(function(){
		...
		try{
			...
			$("#listing").load('pop_matind_load.php',obj,function (){
...
<div id="listing"></div>
	...
	<?php  
		$q = "SELECT mid.*, m.mat_name,m.unit FROM matind mid 
			  INNER JOIN material m ON m.mat_id=mid.mat_id 
			  WHERE matin_id=".$_GET['matin_id'];
		...
		foreach ($rs as $r){
			if (!empty($r['mat_code'])){
            	$uraian=trim($r['mat_code'])." - ".$r['mat_name'];
            } else {
            	$uraian=$r['mat_name'];
            }
	?>
		<tr>	
			<td><?php echo $r['mat_code'] ?></td>
            <td><?php echo $r['mat_name'] ?></td>
            <td><?php echo $r['color'] ?></td>
            <td><?php echo trim($r['unit']) ?></td>
			<td align="right"><?php echo number_format($r['qty'],0) ?></td>
            <td align="right"><?php echo number_format($r['price'],2) ?></td>																					
			<td style="text-align:center;"><input border='0' type='image' src="img/expander.gif" onClick="insert_matind('<?php echo $uraian ?>','<?php echo number_format($r['qty'],0)." ".trim($r['unit']) ?>','<?php echo number_format($r['price'],2) ?>')" /></td>
		</tr>
	<?php } ?>
