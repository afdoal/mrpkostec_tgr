...
$q = "SELECT *,moh.Number AS mohno,c.Number AS cno FROM tbmatout_hd moh LEFT JOIN tbcustomer c ON c.Customer_Code=moh.Buyer ";

if($_POST['dtdari'] != '' && $_POST['dtsampai'] != ''):
	$q .= "WHERE Date BETWEEN '$dtdari' AND '$dtsampai' ";
endif;

$q .=  "ORDER BY Date LIMIT 20";
...
if ($rs){
	foreach ($rs as $r){
		$hasil .= $r['Nomor']."}".ymd2dmy($r['Date'])."}".getWarehouse($r['wh'])."}".getWarehouse($r['To_wh'])."}".$r['ID']."}".$r['cno']."}".$r['NPWP']."}".$r['Customer_Name']."}".$r['Address']."}".$r['TPB']."~";		
	}
} else {
